...
F<sanewall.conf> is the configuration file for sanewall(1), which creates
...
is a variable defined by sanewall that includes all IPs that should not be
...
To make it simple, for sanewall a client is: "a server with all the
...
Sanewall supports the actions defined in the section B<Actions>.
...
Finally, the masquerade helper will turn on SANEWALL_NAT and instruct the
...
action LOG is a sanewall optional rule parameter (see log and loglimit)
...
I<redirect> is used internally by sanewall Helper Commands to redirect 
...
SANEWALL_LOG_OPTIONS and SANEWALL_LOG_LEVEL. You can also change the
...
=item * change SANEWALL_LOG_LEVEL to log at a not so important log-level
...
according to the setting of SANEWALL_LOG_FREQUENCY and SANEWALL_LOG_BURST.
...
defined within interfaces, not routers. Sanewall is "smart" enough to
...
This parameter can be used only in services (client and server
commands) defined within interfaces, not routers. Sanewall is "smart"
...
=head1 Variables that control sanewall
...
=head2 SANEWALL_LOG_LEVEL
=head2 SANEWALL_LOG_OPTIONS
=head2 SANEWALL_LOG_FREQUENCY
=head2 SANEWALL_LOG_BURST

=over 2

I<SANEWALL_LOG_LEVEL> controls the level at which iptables will log 
...
I<SANEWALL_LOG_OPTIONS> controls the way iptables will log things to 
the syslog. The value of this variable is passed as is to iptables,
so use exact iptables parameters.

I<SANEWALL_LOG_FREQUENCY> and I<SANEWALL_LOG_BURST> (added in v1.39 of
sanewall) control the frequency at each each logging rule will write
packets to the syslog. SANEWALL_LOG_FREQUENCY is set to the maximum
average frequency and SANEWALL_LOG_BURST specifies the maximum
...
If I<SANEWALL_NAT> is set to 1, sanewall will load NAT kernel modules
...
=head2 SANEWALL_AUTOSAVE

=over 2

SANEWALL_AUTOSAVE controls the file that will be created when sanewall
...
Default: SANEWALL_AUTOSAVE=""
Example: SANEWALL_AUTOSAVE="/tmp/sanewall-saved.txt" 

=back

=head1 Variables that sanewall offers
...
F</etc/sanewall/sanewall.conf>

=head1 AUTHOR

sanewall written by Costa Tsaousis E<lt>costa@tsaousis.grE<gt>.

Man page written by Marc Brockschmidt E<lt><marc@marcbrockschmidt.deE<gt>.

=head1 SEE ALSO

sanewall(1), iptables(8), bash(1)
