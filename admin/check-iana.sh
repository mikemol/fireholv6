...
then
	logger -p alert -t sanewall "Cannot fetch '${IPV4_ADDRESS_SPACE_URL}'."
	exit 1
fi

if [ ! -s "${spooldir}/${ianafile}" ]
then
	logger -p alert -t sanewall "First fetch of '${IPV4_ADDRESS_SPACE_URL}'."
...
if [ $? -eq 0 ]
then
	logger -p alert -t sanewall "Page '${IPV4_ADDRESS_SPACE_URL}' not changed."
	exit 0
else
	cat <<EOF | sendmail -bm -i -oi -t
From: IANA Monitor on `hostname` <${MYMAIL}>
To: Sanewall Admins <${MYMAIL}>
Errors-To: Sanewall Admins <${MYMAIL}>
...
EOF
	...
	logger -p alert -t sanewall "Changes detected in '${IPV4_ADDRESS_SPACE_URL}'. Mail sent to '${MYMAIL}'."
	mv -f "${spooldir}/${ianafile}.tmp" "${spooldir}/${ianafile}"
	exit 0
fi
